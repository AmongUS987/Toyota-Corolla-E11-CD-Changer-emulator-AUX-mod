...
// Deklaracje zewnętrzne dla komunikatów z AVCLanCDch.cpp
...
void setup() {
    ...
    delay(500);  // Opóźnienie 500 ms (0.5 sekundy)
    if (!avclan.readonly) {  // Opcjonalnie: tylko jeśli nie jest w trybie readonly
        sendRegisterCommand();  // Wyślij komendę rejestracji
    }
}
void loop(){
...
// Funkcja do wysyłania komendy rejestracyjnej i automatycznego startu odtwarzania
void sendRegisterCommand(){
  // Krok 1: Wysłanie komendy rejestracyjnej
  avclan.broadcast = AVC_MSG_DIRECT;
  avclan.masterAddress = 0x0240; // Adres emulatora
  avclan.slaveAddress  = 0x0160; // Poprawny adres panelu
  avclan.dataSize      = 0x05;
  avclan.message[0]    = 0x00;
  avclan.message[1]    = 0x01;
  avclan.message[2]    = 0x12;
  ...
  if (res == 0) {
    ...
    bSerial.println();
    return; // Jeśli rejestracja się nie powiodła, przerwij
  }

  // Krok 2: Wysłanie komunikatów odtwarzania
  ...
  avclan.message[5] = 1; // Dysk 1
  avclan.message[6] = 1; // Ścieżka 1
  avclan.message[7] = 0; // Minuty
  avclan.message[8] = 0; // Sekundy
  ...
  // Krok 3: Aktywacja trybu odtwarzania
